        ...
        <div class="sidebar__item g-filter-item">
            ...
            <div class="sidebar-checkbox ">
                @foreach($item->terms as $key => $term)
                    ...
                    <div class="row y-gap-10 items-center justify-between @if($key > 2 and empty($selected[$item->id])) hide @endif">
                        <div class="col-auto">
                            <label  class="cursor-pointer">
                                <div class="form-checkbox d-flex items-center">
                                    <input @if(in_array($term->id,$selected[$item->id] ?? [])) checked @endif type="checkbox" name="attrs[{{$item->id}}][]" value="{{$term->id}}">
                ...
                @endforeach
                @if(count($item->terms) > 3 and empty($selected[$item->id]))
